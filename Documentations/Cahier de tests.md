...
| CFG-FR-005 | Choix de l'extension du fichier log : XML ou json | Cliquer sur le bouton "Extension log XML" ou "Extension log json" pour passer d'une option à l'autre. | Les logs se génèrent dans le type de fichier choisi. |
| CFG-FR-006 | Choix des extensions de fichiers à chiffrer | Rentrer les extensions de fichiers à chiffrer séparées par des virgules. Exemple : .txt, .json, .jpg  | Tous les fichiers comprenant ces extensions présents dans le dossier source de sauvegarde seront intégralement chiffrer. |
| CFG-FR-007 | Choix d'un logiciel métier | Rentrer le nom d'un logiciel afin d'empêcher tout travail de sauvegarde si ce logiciel est en fonctionnement. Il n'est pas nécessaire de préciser l'extension '.exe' pour que cela fonctionne. | Que le logiciel métier soit lancer avant ou pendant la sauvegarde, celle-ci cessera immédiatement de fonctionner. |
